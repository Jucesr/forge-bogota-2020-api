...
const dbService = require('./utils/mongo')

const {getFactor, sleep, convertObjToDesignAutomationFormat, objectToString, replaceAll} = require('./utils/business')
const {
  createWorkItem,
  getWorkItem,
  postItem,
...
//  Models
const GenerationModel = require('./models/generations');
...
const main = async () => {
    ...
    app.post('/old_generate_model', async (req, res, next) => {
    ...
    console.log('Finished DA', status)
    ...
    console.log('Called BIM360 ', JSON.stringify(item))
    ...
    app.post('/generate_model', async (req, res, next) => {
        ...
        console.log(`Design automation finished ${response}`)
    
    });

    app.get('/model_status/', async (req, res) => {
        let {version_url} = req.query
        version_url = replaceAll(version_url, "\\?", '%3F');
        const {
            AU_CLIENT_ID,
            AU_SECRET_ID,
        } = process.env
        ...
        try{
            const versionItem = await getItem(version_url, credentials.access_token)
            status = versionItem.data.attributes.extension.data.processState 
        }catch(error){
            console.log(error)
            return error;
        }
    ...
    app.get('/generations/', async (req, res) => {
        ...
        console.log(`${generations.length} generations has been sent`)
    })

    app.post('/generation/', async (req, res) => {
        let {name} = req.body

        const gen = await GenerationModel.create(req.body)

        res.status(200).send(gen);
        console.log(`A model has been generated by : ${name}`)
    })

    app.get('/', (req, res) => {
        res.send({
            name: 'Forge Bogota 2020',
            version: '1.0.0'
        })
    })
...
main().then(msg => {
    console.log('App has started correctly')
}).catch(e => {
